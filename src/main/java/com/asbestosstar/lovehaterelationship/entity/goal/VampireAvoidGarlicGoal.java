...
import com.asbestosstar.lovehaterelationship.tag.ModTags;
...
public class VampireAvoidGarlicGoal extends Goal {
		...
		for (int x = -searchRange; x <= searchRange; x++) {
			for (int y = -1; y <= 1; y++) {
				for (int z = -searchRange; z <= searchRange; z++) {
					...
					boolean isDirectGarlicPlant = (block == LoveHateRelationShip.GARLIC_PLANT.get());

					boolean isTaggedGarlic = state.is(ModTags.Blocks.NEOFORGE_GARLIC_CROP)
							|| state.is(ModTags.Blocks.C_GARLIC_CROP);

					boolean isTaggedAnyCrop = state.is(ModTags.Blocks.NEOFORGE_ANY_CROP)
							|| state.is(ModTags.Blocks.C_ANY_CROP) || state.is(ModTags.Blocks.MC_ANY_CROP);

					boolean isRepellent = isDirectGarlicPlant || isTaggedGarlic;

					if (isRepellent) {
						double distanceSq = vampire.distanceToSqr(checkPos.getX() + 0.5, checkPos.getY() + 0.5,
								checkPos.getZ() + 0.5);
	...
	public boolean canContinueToUse() {
		return this.pathToAvoid != null && this.garlicPos != null && vampire.getNavigation().isInProgress();
	}
